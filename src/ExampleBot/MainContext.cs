namespace ExampleBot
{
    ...
    internal class MainContext : Context
    {
        ...
        public MainContext(IContextManager contextManager, IQueuer queuer, string[] gameModes) : base(contextManager)
        {
            ...
            _queuer = queuer;
            _queuer.OnUserStateChanged += (sender, args) =>
            {
                Console.WriteLine($"SERVER: Confirmed user state: {args.State}");
                ConsiderQueueing(this, args);
            };
            _queuer.EnterQueue(_gameMode);
            _queuer.OnGameStarted += (sender, args) => args.Game.OnWaitingForActions += GameTurn;
        }
        ...
        private void ConsiderQueueing(object sender, QueueChangedEventArgs gameEventArgs)
        {
            if (!gameEventArgs.State.IsIngame && !gameEventArgs.State.IsQueued)
                _queuer.EnterQueue(_gameMode);
        }
    }
}