namespace Evaders.Core.Game
{
    ...
    public class Projectile
    {
        ...
        public int DespawnTurn => (int)Math.Ceiling(Game.Settings.ProjectileLifeTimeSec / Game.TimePerFrameSec);
        ...
        {
            if (Game.Turn >= LifeEndTurn || Game.ValidEntities.Any(entity => entity.PlayerIdentifier != PlayerIdentifier && entity.Position.Distance(Position, true) <= (HitboxSize + entity.CharData.HitboxSize) * (HitboxSize + entity.CharData.HitboxSize)))
                Explode();
        }
